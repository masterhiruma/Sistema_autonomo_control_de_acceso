...
def iniciar_aplicacion():
    ...
    # 1. Crear la instancia de la GUI
    #    La GUI ahora es la responsable de iniciar la conexión y los hilos principales.
    app_gui_instancia_global = gui_manager.InterfazGrafica()
    
    # Asignar la referencia de la GUI a los módulos que la necesitan
    if hasattr(arduino_comms, 'asignar_app_gui_referencia'):
        arduino_comms.asignar_app_gui_referencia(app_gui_instancia_global)
    if hasattr(reporting_logging, 'asignar_app_gui_referencia_reporting'):
        reporting_logging.asignar_app_gui_referencia_reporting(app_gui_instancia_global)
    if hasattr(state_machine_logic, 'asignar_app_gui_referencia_fsm'):
        state_machine_logic.asignar_app_gui_referencia_fsm(app_gui_instancia_global)


    # 2. Iniciar el bucle principal de la GUI
...
if __name__ == "__main__":
    ...
    try:
        ...
        if hasattr(state_machine_logic, 'cap_camara_facial') and \
           state_machine_logic.cap_camara_facial and state_machine_logic.cap_camara_facial.isOpened():
            print("Liberando cámara facial (cierre final desde main_app)...")
            state_machine_logic.cap_camara_facial.release()
