...
tiempo_inicio_estado_actual_s = 0
tiempo_puerta_abrio_s = 0
tiempo_sp2_detecto_primera_vez_s = 0
...
# (Sin cambios respecto a v3/v4)
# ==============================================================================
# ... (COPIAR AQUÍ LAS FUNCIONES DE BD COMPLETAS DE LA VERSIÓN ANTERIOR) ...
def inicializar_bd():
...
# (Modificación en generar_reporte_final_dia)
# ==============================================================================
# ... (cargar_estado_diario, guardar_estado_diario, verificar_y_resetear_por_cambio_de_dia, 
#      registrar_evento_acceso_exitoso, registrar_intento_fallido - SIN CAMBIOS RESPECTO A v3/v4) ...
...
def guardar_estado_diario():
    estado_a_guardar = {
        "fecha": fecha_actual_para_conteo, "contador_accesos": contador_accesos_hoy,
        "eventos_accesos_exitosos": eventos_acceso_hoy, "intentos_accesos_fallidos": intentos_fallidos_hoy,
        "intentos_fallidos_por_uid": intentos_fallidos_por_uid, "accesos_recientes_uid": accesos_recientes_uid
    }
    try:
...
def verificar_y_resetear_por_cambio_de_dia():
    ...
    if fecha_actual_para_conteo != fecha_hoy_str:
        print("Detectado cambio de día. Generando reporte anterior y reseteando contadores.")
...
def registrar_evento_acceso_exitoso(usuario_info):
    ...
    evento = {
        "timestamp_acceso": timestamp_str, "nombre_usuario": usuario_info["nombre"],
        "dni_usuario": usuario_info["dni"], "nivel_usuario": usuario_info["nivel"],
        "area_trabajo": usuario_info.get("area", "N/A"), "uid_rfid_usado": usuario_info["uid_rfid"]
    }
    eventos_acceso_hoy.append(evento)
    accesos_recientes_uid[usuario_info["uid_rfid"]] = time.time()
    if usuario_info["uid_rfid"] in intentos_fallidos_por_uid:
...
def registrar_intento_fallido(uid_presentado, usuario_info_si_conocido, motivo_fallo, contar_para_bloqueo_insistencia=True):
    ...
    intento = {
        ...
        "nombre_usuario_detectado": usuario_info_si_conocido["nombre"] if usuario_info_si_conocido else "Desconocido",
        "dni_usuario_detectado": usuario_info_si_conocido.get("dni", "N/A") if usuario_info_si_conocido else "N/A",
    ...
    if contar_para_bloqueo_insistencia and uid_presentado and uid_presentado != "NADA":
        if uid_presentado not in intentos_fallidos_por_uid:
            intentos_fallidos_por_uid[uid_presentado] = {"contador": 0, "nivel_bloqueo": 0, "desbloqueo_hasta": 0}
        ...
        if data_bloqueo_uid.get("desbloqueo_hasta", 0) <= time.time():
            ...
            if data_bloqueo_uid["contador"] >= MAX_INTENTOS_FALLIDOS_UID:
                nivel_actual = data_bloqueo_uid.get("nivel_bloqueo",0)
                nuevo_nivel = min(nivel_actual + 1, 3) 
                data_bloqueo_uid["nivel_bloqueo"] = nuevo_nivel
                duracion_bloqueo_s = TIEMPO_BLOQUEO_UID_NIVEL[nuevo_nivel]
                data_bloqueo_uid["desbloqueo_hasta"] = time.time() + duracion_bloqueo_s
                data_bloqueo_uid["contador"] = 0 
...
def generar_reporte_final_dia(fecha_str, contador_total, lista_eventos_exitosos, lista_eventos_fallidos, bajo_demanda=False):
    ...
    reporte_data_json = {
        "fecha_reporte": fecha_str, "total_accesos_exitosos_dia": contador_total,
    ...
    error_json = None # CORRECCIÓN: Variable para almacenar posible error de JSON
    try:
    ...
    except IOError as e_json: # CORRECCIÓN: Usar variable de excepción local
        error_json = e_json # CORRECCIÓN: Almacenar el error
    ...
    # Generar CSV para accesos exitosos
    ...
    if lista_eventos_exitosos:
        try:
            ...
            if bajo_demanda and app_gui and not error_json: # CORRECCIÓN: Mostrar solo si JSON fue OK o si este es el único reporte
    ...
    # Generar CSV para intentos fallidos
    ...
    if lista_eventos_fallidos:
        try:
            ...
            if bajo_demanda and app_gui and not error_json: # CORRECCIÓN
...
# (Revisada para usar self.modo_escaneo_rfid_para_registro y el _procesar_rfid_llegado_para_formulario corregido)
# ==============================================================================
class InterfazGrafica(tk.Tk):
    def __init__(self):
        ...
        self.title("Sistema de Control de Acceso - v5") # v5
    ...
    def actualizar_campos_horario_form(self, event=None):
        ...
        estado_horario = tk.NORMAL if nivel_seleccionado == "Trabajador" else tk.DISABLED
...
def logica_maquina_estados():
    global puerta_logicamente_abierta, tiempo_puerta_abrio_s, tiempo_sp2_detecto_primera_vez_s, datos_hardware, app_gui
    ...
    while hilo_maquina_estados_activo:
        ...
        with lock_datos_hardware: 
            dist_sp1 = datos_hardware["sp1_distancia"]
            dist_sp2 = datos_hardware["sp2_distancia"]
        ...
        if estado_actual_sistema == EstadoSistema.REPOSO:
            ...
            if puerta_logicamente_abierta:
                cambiar_estado(EstadoSistema.CERRANDO_PUERTA, "Error: Puerta abierta en reposo, cerrando."); continue
            if 0 < dist_sp1 < UMBRAL_DETECCION_SP1_CM:
                enviar_comando_a_arduino("SOLICITAR_LECTURA_RFID")
                cambiar_estado(EstadoSistema.ESPERANDO_VALIDACION_RFID, "Presente su tarjeta RFID...")
        
        elif estado_actual_sistema == EstadoSistema.ESPERANDO_VALIDACION_RFID:
            if not (0 < dist_sp1 < UMBRAL_DETECCION_SP1_CM):
                cambiar_estado(EstadoSistema.REPOSO, "Usuario se retiró."); continue
            if tiempo_actual_s - tiempo_inicio_estado_actual_s > TIMEOUT_PRESENTACION_RFID_S:
                uid_timeout = uid_actual_arduino if uid_actual_arduino != "NADA" and uid_actual_arduino != ultimo_rfid_procesado_acceso else None
                registrar_intento_fallido(uid_timeout, None, "Timeout Presentación RFID") # Contar_para_bloqueo es True por defecto
                cambiar_estado(EstadoSistema.ACCESO_DENEGADO_TEMPORAL, "Tiempo agotado para RFID."); continue
            ...
            if uid_actual_arduino != "NADA" and uid_actual_arduino != ultimo_rfid_procesado_acceso:
                ...
                data_bloqueo = intentos_fallidos_por_uid.get(uid_actual_arduino)
                if data_bloqueo and data_bloqueo.get("desbloqueo_hasta", 0) > tiempo_actual_s:
                    tiempo_restante_bloqueo = int(data_bloqueo["desbloqueo_hasta"] - tiempo_actual_s)
                    mensaje_bloqueo = f"UID {uid_actual_arduino} bloqueado. Intente en {max(0,tiempo_restante_bloqueo) // 60}m {max(0,tiempo_restante_bloqueo) % 60}s."
                    print(mensaje_bloqueo) 
                    registrar_intento_fallido(uid_actual_arduino, None, "Intento con UID bloqueado", contar_para_bloqueo_insistencia=False)
                    cambiar_estado(EstadoSistema.SISTEMA_BLOQUEADO_UID, mensaje_bloqueo); continue 

                usuario_info = obtener_usuario_por_rfid_bd(uid_actual_arduino)
                acceso_concedido = False; mensaje_denegado = "Acceso Denegado."; motivo_fallo_log = "Tarjeta no reconocida"

                if usuario_info:
                    ts_ultimo_acceso = accesos_recientes_uid.get(uid_actual_arduino)
                    if ts_ultimo_acceso and (tiempo_actual_s - ts_ultimo_acceso < TIEMPO_COOLDOWN_ACCESO_S):
                        tiempo_restante_cooldown = int(TIEMPO_COOLDOWN_ACCESO_S - (tiempo_actual_s - ts_ultimo_acceso))
                        mensaje_denegado = f"Acceso reciente ({usuario_info['nombre']}). Intente en {max(0,tiempo_restante_cooldown) // 60}m {max(0,tiempo_restante_cooldown) % 60}s."
                        motivo_fallo_log = "Cooldown Anti-Passback activo"
                        registrar_intento_fallido(uid_actual_arduino, usuario_info, motivo_fallo_log, contar_para_bloqueo_insistencia=False) 
                        enviar_comando_a_arduino("LED_ROJO_ON")
                        cambiar_estado(EstadoSistema.ACCESO_DENEGADO_TEMPORAL, mensaje_denegado); continue 
                    
                    print(f"Usuario encontrado: {usuario_info['nombre']}, Nivel: {usuario_info['nivel']}")
                    motivo_fallo_log = f"Nivel: {usuario_info['nivel']}" 
                    if usuario_info["nivel"] == "Admin": acceso_concedido = True
                    elif usuario_info["nivel"] == "Trabajador":
                        if verificar_horario_trabajador(usuario_info["h_inicio"], usuario_info["h_fin"]): acceso_concedido = True
                        else: mensaje_denegado = "Acceso Denegado: Fuera de horario laboral."; motivo_fallo_log += " - Fuera Horario"
                    elif usuario_info["nivel"] == "Visitante":
                        if verificar_horario_visitante(): acceso_concedido = True
                        else: mensaje_denegado = "Acceso Denegado: Fuera de horario de visita."; motivo_fallo_log += " - Fuera Horario"
                
                if acceso_concedido:
                    registrar_evento_acceso_exitoso(usuario_info); enviar_comando_a_arduino("LED_VERDE_ON")
                    cambiar_estado(EstadoSistema.ABRIENDO_PUERTA, f"Acceso Concedido: {usuario_info['nombre']}")
                else: 
                    print(f"ACCESO DENEGADO: {mensaje_denegado}")
                    se_bloqueo_uid = registrar_intento_fallido(uid_actual_arduino, usuario_info, motivo_fallo_log, contar_para_bloqueo_insistencia=True) 
                    if se_bloqueo_uid:
                         data_bloqueo_actual = intentos_fallidos_por_uid.get(uid_actual_arduino, {})
                         tiempo_rest_bl = int(data_bloqueo_actual.get("desbloqueo_hasta", tiempo_actual_s) - tiempo_actual_s)
                         msg_bl = f"UID {uid_actual_arduino} bloqueado (3 intentos). Intente en {max(0,tiempo_rest_bl) // 60}m {max(0,tiempo_rest_bl) % 60}s."
                         cambiar_estado(EstadoSistema.SISTEMA_BLOQUEADO_UID, msg_bl)
                    else:
                        enviar_comando_a_arduino("LED_ROJO_ON")
                        cambiar_estado(EstadoSistema.ACCESO_DENEGADO_TEMPORAL, mensaje_denegado)
