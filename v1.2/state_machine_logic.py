...
def cambiar_estado(nuevo_estado, mensaje_gui=None):
    """Cambia el estado del sistema y actualiza la GUI"""
    global estado_actual_sistema, tiempo_inicio_estado_actual_s
    
    # Actualizar el estado y el tiempo de inicio
    estado_actual_sistema = nuevo_estado
    tiempo_inicio_estado_actual_s = time.time()
    
    # Determinar el mensaje si no se proporciona uno
    if mensaje_gui is None:
        mensaje_gui = {
            ...
            EstadoSistema.ESPERANDO_VALIDACION_FACIAL: "Mire a la cámara para validación facial...",
            EstadoSistema.ABRIENDO_PUERTA: "Acceso concedido, abriendo puerta...",
            EstadoSistema.PERSONA_CRUZANDO: "Persona cruzando, espere...",
            EstadoSistema.ACCESO_DENEGADO_TEMPORAL: "Acceso denegado temporalmente",
            EstadoSistema.SISTEMA_BLOQUEADO_UID: "Sistema bloqueado para este UID",
            EstadoSistema.ALERTA_ERROR_CRUCE: "Alerta manejada",
            EstadoSistema.CERRANDO_PUERTA: "Sistema en reposo",
            EstadoSistema.EMERGENCIA_ACTIVA: "¡¡MODO EMERGENCIA ACTIVADO!!",
            EstadoSistema.ESPERANDO_VALIDACION_FACIAL: "Espere, buscando rostro..."
        }.get(nuevo_estado, "Estado desconocido")
    
    # Actualizar la GUI si está disponible
    if app_gui_ref_fsm and hasattr(app_gui_ref_fsm, 'actualizar_mensaje'):
        app_gui_ref_fsm.actualizar_mensaje(mensaje_gui)
    
    # Actualizar el estado en la GUI si está disponible
    if hasattr(app_gui_ref_fsm, 'labels_estado_principal'):
        if 'lbl_estado_sistema_valor' in app_gui_ref_fsm.labels_estado_principal:
            app_gui_ref_fsm.labels_estado_principal['lbl_estado_sistema_valor'].configure(
                text=nuevo_estado.value
            )
    ...
    while hilo_maquina_estados_activo: # Este flag debe ser importado o definido en este módulo
        ...
        if not arduino_comms.is_arduino_conectado(): 
            ...
            if cap_camara and cap_camara.isOpened():
                ...
                if app_gui_ref_fsm and hasattr(app_gui_ref_fsm, 'lbl_camera_feed'): # Limpiar la visualización de la cámara en la GUI
                    app_gui_ref_fsm.lbl_camera_feed.config(image='', text="Cámara OFF")
        ...
        if estado_actual_sistema == EstadoSistema.EMERGENCIA_ACTIVA:
            if grabando_video_emergencia and cap_camara and cap_camara.isOpened() and video_writer_emergencia is not None:
                ...
                if ret:
                    ...
                    if app_gui_ref_fsm and hasattr(app_gui_ref_fsm, 'mostrar_frame_camara'): # Mostrar en la GUI
                        app_gui_ref_fsm.mostrar_frame_camara(frame_e)
        ...
        if estado_actual_sistema == EstadoSistema.REPOSO:
            ...
            if app_gui_ref_fsm and hasattr(app_gui_ref_fsm, 'mostrar_frame_camara'):
                app_gui_ref_fsm.mostrar_frame_camara(frame)
            ...
            if not face_locations:
                ...
                if usuario_identificado_paso_previo and protocolo_seleccionado_actual["facial"]:
                    if usuario_identificado_paso_previo.get("facial_encoding_array") is None:
                        ...
                        if app_gui_ref_fsm and hasattr(app_gui_ref_fsm, 'lbl_camera_feed'): app_gui_ref_fsm.lbl_camera_feed.config(image='', text="Cámara OFF")
