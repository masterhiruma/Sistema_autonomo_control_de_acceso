...
def listar_y_probar_camaras_sin_gui():
    camaras_disponibles = []
    for i in range(10): # Probar hasta 10 índices de cámara
        cap = cv2.VideoCapture(i, cv2.CAP_DSHOW)
        if cap.isOpened():
            camaras_disponibles.append(i)
            cap.release() # Liberar la cámara inmediatamente
        else:
            # Si un índice no es válido, a menudo los siguientes tampoco lo son,
            # pero seguimos un poco más por si acaso (ej. un índice intermedio no válido).
            pass
